...
static const char http_index_html[] = "<html><head><title>Congrats!:%d</title></head><body><h1>Welcome to our lwIP HTTP server!</h1><p>This is a small test page. Called:%d</body></html>";
static int call = 0;
static char htmlBuff[256];
static void http_server_serve(struct netconn *conn) {
  ...
  if (err == ERR_OK) {
    ...
    if (buflen>=5 &&
      ...
      chsnprintf(htmlBuff,sizeof(htmlBuff),http_index_html,++call,call);
      /* Send our HTML page */
      netconn_write(conn, htmlBuff, strlen(htmlBuff), NETCONN_NOCOPY);
    }
  }
