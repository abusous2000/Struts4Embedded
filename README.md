...
It should be *noted* that these use cases could be easily modified or extended to fit other IOT projects.
...
- If you decide to use *[FlySky-i6s]*(https://github.com/abusous2000/Struts4Embedded/tree/master/source/Controls/PPMFrameDecoder) to send control signals via RC, please configure its output to be PPM & i-Bus in the system config menu & make sure it is already bounded to the receiver. All you need is ground, 5V, and the PPM line. Note that current configuration by default ignores every other 9 frames, which could be adjusted. The frame are coming too fast; this is meant to reach steady state quickly else flickering transition states will be reported. Also note that if use SD-Card; you may have to power it externally and not from the board itself. The is true in the case STM32F4-Discovery.
